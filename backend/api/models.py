...
async def ai_generate_model(req: AIGenerateRequest):
    ...
    system = (
        ...
        "可用材质：颜色(RGBA)/metallic/roughness/emissive/texture(程序化纹理)\n"
        "可用操作：CSG布尔(subtract挖洞/intersect交集/union合并)\n"
        + base_ctx +
        "\n═══ 核心工作流：先深度分析，再拼装 ═══\n"
        ...
        "  例：补子=thin box+texture:fabric+金色(1零件+纹理暗示刺绣)\n"
        "  例：盔甲=box+metallic=0.8,roughness=0.3,texture:metal_brush(金属质感+拉丝纹理)\n"
        "  例：拱门=box+csg:{subtract,cylinder}(1零件挖洞成拱门)\n"
        "  例：花瓶=lathe+profile轮廓线(1零件旋转体)\n"
        "  例：徽章=extrude+cross_section截面(1零件挤出体)\n"
        "  例：砖墙=box+texture:brick(1零件+纹理顶10个纯色box)\n"
        ...
        "- lathe：旋转体(花瓶/柱子/碗/杯子/帽子圆顶)→用profile定义轮廓\n"
        "- extrude：挤出体(徽章/文字/截面形状)→用cross_section定义2D截面\n"
        "\n═══ 程序化纹理(1零件+纹理>10个纯色零件) ═══\n"
        "- brick：砖墙纹理(城墙/房屋/烟囱)\n"
        "- wood：木纹纹理(家具/门/柱子)\n"
        "- fabric：布纹纹理(官服/长袍/旗帜)\n"
        "- metal_brush：拉丝金属(盔甲/武器/机械)\n"
        "- checker：棋盘格(地板/桌面)\n"
        "- dragon_scale：龙鳞(龙/蛇/奇幻生物)\n"
        "- tile：瓷砖(屋顶/地板/墙面)\n"
        "\n═══ CSG布尔运算(加减法建模) ═══\n"
        "- subtract：挖洞(拱门=box-cylinder | 窗户=墙-box | 镂空)\n"
        "- intersect：交集(截取形状)\n"
        "- union：合并(无缝连接)\n"
        "\n═══ 拼装规范 ═══\n"
        "- 原点(0,0,0)=模型底部中心，Y朝上\n"
        ...
        "\n═══ 高级技巧 ═══\n"
        "- 纹理替代：1个带texture的box > 10个纯色box拼砖墙/布纹/木纹\n"
        "- CSG挖洞：拱门/窗户/镂空用subtract，1零件搞定以前需要5+零件的形状\n"
        "- lathe旋转体：花瓶/柱子/碗用profile一条线旋转，比拼cylinder+sphere更自然\n"
        "- extrude挤出：徽章/标志/截面用cross_section挤出，比拼box更精准\n"
        "- 叠加法：thin box+texture暗示图案(补子+fabric纹理/铠甲+metal_brush纹理)\n"
        "- 姿态表达：旋转零件表达动作(手臂前伸=arm cylinder旋转90°朝前)\n"
        "- 层次感：外层零件略大于内层，形成衣袍/铠甲覆盖效果\n"
        "- 材质区分：同色不同材质靠metallic/roughness区分\n"
