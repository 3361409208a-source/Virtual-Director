...
def _actor_system(director: dict, actor_ids: list[str] | None = None) -> str:
    ...
    return (
        ...
        "【骨骼动画规范（bone_anim + sub_tracks）】：\n"
        "═══ humanoid 演员 ═══\n"
        "每个关键帧必须填写 bone_anim 字段，渲染器将自动生成对应肢体摆动：\n"
        "  · 站立/等候：bone_anim='idle'\n"
        "  · 正常步行（速度 <3m/s）：bone_anim='walk'\n"
        "  · 跑步/冲刺（速度 ≥3m/s）：bone_anim='run'\n"
        "  · 被撞飞/倒地/失控：bone_anim='ragdoll'\n"
        "示例：人物走路被车撞飞 → 走路阶段所有帧设 walk，被撞瞬间帧设 ragdoll。\n\n"
        "═══ car/plane 演员 ═══\n"
        "sub_tracks 必须包含轮子/螺旋桨旋转。key 与 asset manifest 的 parts.name 一致。\n"
        "车轮滚动公式：每移动 1 米，rotation.x 累积增加 ~57°（360/(2π*r), r≈1m）；\n"
        "如有多个轮子（wheel_fl/wheel_fr/wheel_rl/wheel_rr）每个都必须填写；\n"
