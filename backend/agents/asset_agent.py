...
def run_asset_agent(prompt: str, director: dict, progress_cb=None, token_cb=None) -> dict:
    ...
    if remaining:
        ...
        system = (
            ...
            "Available shapes: box, sphere, cylinder, cone, capsule, lathe, extrude.\n"
            "Available material props: color(RGBA), metallic(0-1), roughness(0-1), emissive(glow), texture(procedural).\n"
            "Available operations: CSG boolean (subtract/intersect/union).\n"
            "Respond in Chinese.\n\n"
            f"Asset brief: {brief}\n"
            ...
            "   - Suggestive features -> 1-2 parts + precise color + texture to hint.\n"
            "   - Unrepresentable -> skip, budget to identity features.\n"
            "3. Part budget: 12-25 parts per actor, identity 60%, suggestive 30%, base 10%.\n\n"
            "Shape guide: sphere=heads/balls; cylinder=wheels/barrels/limbs; box=body/chassis/panels; cone=hats/tips/skirts; capsule=torso/limbs/rounded; lathe=vase/pillar/bowl(profile); extrude=badge/emblem(cross_section).\n"
            "Texture types: brick/wood/fabric/metal_brush/checker/dragon_scale/tile - 1 textured part > 10 plain parts.\n"
            "CSG: subtract=hole(arch=box-cylinder), intersect=clip, union=merge.\n"
