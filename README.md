...
https://github.com/user-attachments/assets/86f2b058-8b2c-467f-ac27-e123b4d1a3e1
...
| **动画导演 (Actor)** | 每个角色的 3D 运动轨迹关键帧（位置、旋转、缩放），**支持多演员并发并行规划** |
...
系统支持三种建模模式，确保在任何情况下都能产出可用的 3D 资产：

* **快速草模 (Procedural)**：找不到合适模型时，用基础几何体（Box / Sphere / Cylinder 等）**程序化拼装**，零依赖，极速响应。
* **开源高精 (Open-source / Shap-E)**：集成了 OpenAI 的 **Shap-E** 轻量级本地模型（也可选接腾讯 Hunyuan3D-2），支持 Text-to-3D 生成具有整体感和纹理的 3D 模型。
* **自动回退 (Auto)**：优先尝试高精生成，若环境不支持（如显存不足）则自动回退到程序化建模，保证生成流程不中断。

### ⚡ 并行加速优化

针对复杂场景，系统实现了深度并行优化：

* **Agent 级并行**：五大创作 Agent 同时启动，互不阻塞。
* **智能体内部并行**：**动画导演 (Actor)** 智能体支持将多个演员的动作规划拆分为独立子任务并发执行，大幅缩短长视频生成的等待时间。
